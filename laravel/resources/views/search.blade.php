...
<div class="flex-col px-6 py-8 bg-dark border-b border-border">
    <div class="flex items-center gap-2 text-xs mb-2 max-w-7xl mx-auto">
        ...
        <span class="flex items-center gap-2"><span>/</span><span>Search</span></span>
    </div>
    ...
    @if ($q !== '')
        <p class="max-w-7xl mx-auto text-placeholder text-sm">
            Found {{ $movies->count() + $souvenirs->count() }} result(s) across movies and souvenirs.
        </p>
    @endif
</div>
    ...
    @else
        ...
        @if ($movies->isNotEmpty())
            <section class="mb-10">
                <h2 class="text-xl font-bold mb-4">
                    Movies
                    <span class="text-sm font-normal text-placeholder ml-2">{{ $movies->count() }} result(s)</span>
                </h2>
        ...
        @if ($souvenirs->isNotEmpty())
            <section>
                <h2 class="text-xl font-bold mb-4">
                    Souvenirs
                    <span class="text-sm font-normal text-placeholder ml-2">{{ $souvenirs->count() }} result(s)</span>
                </h2>
                <ul class="grid grid-cols-2 gap-3 tablet:grid-cols-3 desktop:grid-cols-5">
                    @foreach ($souvenirs as $souvenir)
                        <li class="group bg-dark rounded-2xl overflow-hidden border border-border shadow-[0_14px_36px_rgba(0,0,0,.35)] transition-all duration-300 hover:bg-button hover:border-accent hover:scale-105">
                            <a href="{{ route('souvenirs.show', \Illuminate\Support\Str::slug($souvenir->name)) }}" class="block relative h-full">
                                <div class="absolute inset-0 bg-gradient-to-b from-accent/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity z-10"></div>
                                ...
                                <div class="flex flex-col gap-1 p-3">
                                    <div class="flex items-center justify-between gap-3">
                                        <span class="min-w-0 truncate font-semibold">{{ $souvenir->name }}</span>
                                        <span class="shrink-0 text-xs font-bold text-accent">{{ number_format($souvenir->price, 2) }}€</span>
                                    </div>
                                    <div class="text-placeholder text-xs">{{ $souvenir->category }}</div>
                                    @if ($souvenir->movie_title)
                                        <div class="text-placeholder text-xs truncate">{{ $souvenir->movie_title }}</div>
                                    @endif
                                </div>
                            </a>
                        </li>
