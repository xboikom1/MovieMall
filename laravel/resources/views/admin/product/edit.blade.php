    ...
    <main class="mx-auto max-w-7xl px-4 py-8 tablet:px-6">
        <div class="flex items-center gap-2 text-xs text-placeholder mb-6">
            ...
            <span class="text-text"
                >Edit {{ $type === 'movie' ? 'Movie' : 'Souvenir' }}</span
            >
        </div>

        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">
                Edit {{ $type === 'movie' ? 'Movie' : 'Souvenir' }}
            </h1>
            <a
                href="{{ route('admin.dashboard') }}"
        ...
        @if ($errors->any())
            <div
                class="mb-4 rounded-xl border border-red-500/30 bg-red-900/30 px-4 py-3 text-sm text-red-200"
            >
            ...
            <div class="flex flex-col gap-5">
                ...
                >
                    ...
                    <form
                        id="main-form"
                        method="POST"
                        action="{{ route('admin.product.update', ['type' => $type, 'id' => $product->id]) }}"
                    ...
                    >
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="relative">
                                    <span
                                        class="absolute left-3.5 top-1/2 -translate-y-1/2 text-placeholder text-sm"
                                        >€</span
                                    >
                                    <input
                                        id="price"
                    ...
                    @else
                        <div class="grid grid-cols-2 tablet:grid-cols-4 gap-3">
                            @foreach ($images as $image)
                                <div
                                    class="group relative rounded-xl overflow-hidden border border-border bg-button aspect-square"
                                >
                                    <img
                                        src="{{ $image->url }}"
                                        alt=""
                                    ...
                                    @if ($image->is_primary)
                                        <span
                                            class="absolute top-1.5 left-1.5 rounded-md bg-accent px-1.5 py-0.5 text-xs font-semibold"
                                            >Main</span
                                        >
                                    @else
                                        <form
