        ...
        >
            ...
            >
                <div class="flex gap-2">
                    <a
                        href="{{ route('admin.orders', array_filter(['status' => 'all', 'search' => $search])) }}"
                        class="rounded-lg px-3 py-1.5 text-xs transition {{ $status === 'all' ? 'bg-accent font-semibold' : 'border border-border bg-button hover:border-accent' }}"
                        >All</a
                    >
                    <a
                        href="{{ route('admin.orders', array_filter(['status' => 'paid', 'search' => $search])) }}"
                        class="rounded-lg px-3 py-1.5 text-xs transition {{ $status === 'paid' ? 'bg-accent font-semibold' : 'border border-border bg-button hover:border-accent' }}"
                        >Paid</a
                    >
                    <a
                        href="{{ route('admin.orders', array_filter(['status' => 'draft', 'search' => $search])) }}"
                        class="rounded-lg px-3 py-1.5 text-xs transition {{ $status === 'draft' ? 'bg-accent font-semibold' : 'border border-border bg-button hover:border-accent' }}"
                        >Draft</a
                    >
                </div>
                <form
                ...
                >
                    ...
                    @if ($search)
                        <a
                            href="{{ route('admin.orders', ['status' => $status]) }}"
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        @forelse ($orders as $order)
                            ...
                            >
                                <td
                                    class="px-5 py-3 font-mono text-xs text-placeholder"
                                >
                                ...
                                <td
                                    class="px-5 py-3 text-placeholder text-xs whitespace-nowrap"
                                >
                                    {{ \Carbon\Carbon::parse($order->created_at)->format('M j, Y · H:i') }}
                                </td>
                                <td
                                    class="px-5 py-3 capitalize text-placeholder text-xs"
                                >
                                    {{ $order->delivery_type ?? '—' }}
                                </td>
