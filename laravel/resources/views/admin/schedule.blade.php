        ...
        >
            ...
            <div class="flex items-center gap-3">
                ...
                <a
                    ...
                    class="rounded-lg border border-accent bg-accent/10 px-3 py-2 text-sm font-semibold text-accent"
                    >Schedule</a
                >
                <a
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 tablet:px-6">
        ...
        <div class="grid grid-cols-1 gap-6 desktop:grid-cols-3">
            ...
            <div class="desktop:col-span-1">
                <div
                    class="rounded-2xl border border-border bg-dark p-6 shadow-[0_14px_36px_rgba(0,0,0,.35)]"
                >
                    <h2 class="mb-5 text-lg font-bold">Add Screening</h2>

                    <form
                        method="POST"
                        action="{{ route('admin.schedule.store', array_filter(['movie_id' => $movieFilter])) }}"
                    ...
                    >
                        ...
                        <div>
                            <label
                                class="mb-1 block text-xs font-semibold uppercase tracking-widest text-placeholder"
                                >Movie</label
                            >
                            <select
                                name="movie_id"
                            ...
                            >
                                ...
                                @foreach ($movies as $m)
                                    <option
                                        value="{{ $m->id }}"
                                        {{ old('movie_id', $movieFilter) == $m->id ? 'selected' : '' }}
                                    >
                                        {{ $m->title }}
                                    </option>
                                @endforeach
                ...
                >
                    ...
                    @else
                        <table class="w-full text-sm">
                            <thead>
                                <tr
                                    class="border-b border-border text-xs uppercase tracking-widest text-placeholder"
                                >
                            ...
                            <tbody>
                                @foreach ($slots as $slot)
                                    <tr
                                        class="border-b border-border last:border-0 hover:bg-button/50 transition"
                                    >
                                        <td class="px-5 py-3 font-semibold">
                                            {{ $slot->movie_title }}
                                        </td>
                                        <td class="px-5 py-3 text-placeholder">
                                            {{ $slot->hall_name }}
                                        </td>
                                        <td class="px-5 py-3">
                                            {{ \Carbon\Carbon::parse($slot->starts_at)->format('M j, Y') }}
                                        </td>
                                        <td class="px-5 py-3">
                                            {{ \Carbon\Carbon::parse($slot->starts_at)->format('H:i') }}
                                        </td>
                                        <td class="px-5 py-3 text-placeholder">
                                            {{ \Carbon\Carbon::parse($slot->ends_at)->format('H:i') }}
                                        </td>
                                        <td class="px-5 py-3 text-right">
                                            <form
                                                method="POST"
                                                action="{{ route('admin.schedule.destroy', $slot->id) }}"
                                                onsubmit="
                                                    return confirm(
