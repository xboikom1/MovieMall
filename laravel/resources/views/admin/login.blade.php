    ...
    <html lang="en">
        ...
        <body class="min-h-screen bg-bg text-text">
            <header class="border-b border-border bg-dark shadow-[0_14px_36px_rgba(0,0,0,0.4)]">
                <div class="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-4 tablet:px-6">
                    <div class="flex items-center gap-3 tablet:gap-6">
                        <a href="{{ route('home') }}" class="flex items-center rounded-xl bg-accent px-4 py-2 font-semibold tablet:text-lg"> MovieMall </a>
                    </div>
            ...
            <main class="mx-auto flex min-h-[calc(100vh-76px)] max-w-7xl items-center justify-center px-4 py-8 tablet:px-6 tablet:py-14">
                <section class="w-full max-w-xl rounded-3xl border border-border bg-dark p-5 shadow-[0_14px_36px_rgba(0,0,0,.4)] tablet:max-w-2xl tablet:p-10">
                    <h1 class="mb-6 text-2xl font-bold tablet:mb-8 tablet:text-4xl">Admin sign in</h1>

                    @if (session('status'))
                        <p class="mb-4 text-sm text-green-400">{{ session('status') }}</p>
                    @endif

                    <form method="POST" action="{{ route('admin.login') }}" class="mx-auto flex max-w-xl flex-col gap-4 tablet:gap-6">
                        @csrf
                        ...
                        <div class="flex items-center justify-between">
                            <label class="flex cursor-pointer items-center gap-2 text-sm text-placeholder">
                                <input id="remember_me" name="remember" type="checkbox" class="rounded border-border bg-button text-accent focus:ring-accent" />
                                Remember me
                            </label>
                        </div>
