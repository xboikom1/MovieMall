...
<body class="min-h-screen bg-bg text-text">
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 tablet:px-6 tablet:py-12">
        ...
        @if (session('status') === 'profile-updated')
            <div
                class="mb-6 rounded-xl border border-green-600 bg-green-800/30 px-4 py-3 text-sm text-green-300"
            >
        ...
        {{-- Profile card --}}
        <section
            class="rounded-3xl border border-border bg-dark p-5 shadow-[0_14px_36px_rgba(0,0,0,.4)] tablet:p-10"
        >
            <div
                class="grid grid-cols-1 gap-8 desktop:grid-cols-[320px_1fr] desktop:gap-10"
            >
                {{-- Avatar --}}
                <aside class="flex flex-col items-center gap-4">
                    <div
                        class="grid h-44 w-44 place-items-center rounded-full border border-border bg-button text-center shadow-[0_14px_36px_rgba(0,0,0,.35)]"
                    >
                        @if ($user->avatar_url)
                            <img
                                src="{{ $user->avatar_url }}"
                    ...
                    >
                        ...
                        >
                            @csrf
                            <label
                                class="cursor-pointer rounded-xl border border-border bg-bg px-4 py-2 text-sm font-semibold transition hover:border-accent hover:text-accent"
                            >
                                Upload New
                                <input
                                    type="file"
                        ...
                        @if ($user->avatar_url)
                            ...
                            >
                                ...
                                <button
                                    type="submit"
                                    class="rounded-xl border border-border bg-bg px-4 py-2 text-sm font-semibold transition hover:border-red-500 hover:text-red-400"
                                >
                                    Delete Avatar
                                </button>
                            </form>
                    ...
                    >
                        ...
                        <div class="grid grid-cols-1 gap-5 tablet:grid-cols-2">
                            <div>
                                <label
                                    for="first_name"
                                    class="mb-2 block text-xs font-semibold uppercase tracking-widest"
                                    >First Name</label
                                >
                                <input
                                    id="first_name"
        ...
        {{-- Delivery Addresses --}}
        <section
            class="mt-8 rounded-3xl border border-border bg-dark p-5 shadow-[0_14px_36px_rgba(0,0,0,.4)] tablet:p-10"
        >
            ...
            <details
                class="mb-6"
                {{ $errors->hasAny(['street', 'city', 'postal_code', 'country']) ? 'open' : '' }}
            >
                <summary
                    class="mb-4 w-fit cursor-pointer list-none rounded-xl border border-border bg-bg px-4 py-2 text-sm font-semibold transition hover:border-accent hover:text-accent"
                >
                    + Add New
                </summary>
                ...
                <div class="rounded-2xl border border-border bg-bg p-5">
                    <h3
                        class="mb-4 text-sm font-semibold uppercase tracking-widest text-placeholder"
                    >
                        New Address
                    </h3>
                    <form
            ...
            @else
                <ul class="flex flex-col gap-4">
                    @foreach ($addresses as $address)
                        <li
                            class="flex items-start justify-between gap-4 rounded-2xl border border-border bg-bg p-4"
                        >
