...
<head>
    ...
    <style>
        @media (max-width: 1023px) {
            ...
            #filter-toggle:checked ~ .filter-label .filter-chevron {
    ...
    <div class="flex-col px-6 py-8 bg-dark border-b border-border">
        <div class="flex items-center gap-2 text-xs mb-2 max-w-7xl mx-auto">
            ...
            <span class="flex items-center gap-2"
                ><span>/</span><span>Souvenirs</span></span
            >
        </div>
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 tablet:px-6 tablet:py-8">
        ...
        >
            ...
            >
                ...
                >
                    <!-- hidden checkbox — drives mobile toggle, no name so not submitted -->
                    ...
                    >
                        ...
                        <!-- category -->
                        <h3 class="text-placeholder text-xs font-bold mt-4">
                            CATEGORY
                        </h3>
                        ...
                        <div class="flex flex-col text-sm my-1 gap-1.5">
                            @foreach ($franchises as $franchise)
                                <label class="flex items-center gap-1 min-w-0">
                                    ...
                                    />
                                    <span
                                        class="truncate cursor-pointer transition hover:text-accent"
                ...
                <section class="min-w-0 flex-1">
                    <div class="flex items-center justify-between gap-3 mb-4">
                        <p class="text-sm text-placeholder">Showing <span class="text-text font-bold">{{ $souvenirs->firstItem() ?? 0 }}–{{ $souvenirs->lastItem() ?? 0 }}</span> of <span class="text-text font-bold">{{ $souvenirs->total() }}</span> souvenirs</p>
                    ...
                    @else
                        ...
                        >
                            @foreach ($souvenirs as $s)
                                <li
                                    class="group bg-dark rounded-xl overflow-hidden border border-border shadow-[0_14px_36px_rgba(0,0,0,.35)] transition-all duration-300 hover:bg-button hover:border-accent hover:scale-[1.03]"
                                >
                                    <a
                                    ...
                                    >
                                        ...
                                        >
                                            <img
                                                src="{{ $s->image ?? '/images/grandpa/grandpa_figurine.jpg' }}"
                                                alt="{{ $s->name }}"
                                                class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                                            />
                                        </div>
                                        <div
                                            class="flex flex-col gap-1 p-2 pb-3"
                                        >
                                            <div
                        ...
                        >
                            ...
                            >
                                ...
                                @else
                                    <a
                                        href="{{ $souvenirs->previousPageUrl() }}"
                                ...
                                @foreach (range(1, $souvenirs->lastPage()) as $p)
                                    ...
                                    @elseif ($p <= 3 || $p > $souvenirs->lastPage() - 3 || ($p >= $souvenirs->currentPage() - 1 && $p <= $souvenirs->currentPage() + 1))
                                        <a
                                            href="{{ $souvenirs->url($p) }}"
                                            class="rounded-lg border border-border bg-button px-3 py-1.5 transition hover:border-accent hover:bg-accent"
                                            >{{ $p }}</a
                                        >
