        ...
        <div class="min-w-0 flex-1 flex flex-col gap-10">
            ...
            <section>
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <span class="rounded-full bg-button border border-border px-2.5 py-0.5 text-xs text-placeholder">{{ count($tickets) }} films</span>
                </div>
                <div class="flex flex-col gap-4">
                    ...
                    <div class="rounded-2xl border border-border bg-dark shadow-[0_14px_36px_rgba(0,0,0,.35)] overflow-hidden">
                        <div class="flex gap-4 p-4 tablet:p-5">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2">
                                    <div class="min-w-0">
                                        <a href="{{ $item['url'] }}" class="block line-clamp-2 font-semibold hover:text-accent transition tablet:text-lg">{{ $item['name'] }}</a>
                                        <div class="flex flex-wrap items-center gap-2 mt-1 text-xs text-placeholder">
                                            <span>{{ $item['genre'] }}</span>
                                            <span>·</span>
                                            <span>{{ $item['year'] }}</span>
                                            <span>·</span>
                                            <span class="text-xs text-rating">★ {{ $item['rating'] }}</span>
                                        </div>
                                    </div>
                                    <form method="POST" action="{{ route('cart.item.remove') }}" class="shrink-0">
                                        @csrf
                                        <input type="hidden" name="type" value="{{ $item['type'] }}">
                                        <input type="hidden" name="reference_id" value="{{ $item['reference_id'] }}">
                                        <input type="hidden" name="options" value="{{ json_encode($item['options'] ?? []) }}">
                                        <button type="submit" class="text-placeholder hover:text-accent transition leading-none mt-0.5" title="Remove">✕</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-border bg-button/40 px-4 py-4 tablet:px-5 flex flex-col gap-3">
                            <div class="text-sm text-placeholder">{{ $item['schedule'] }}</div>
                            ...
                            <div class="grid grid-cols-[1fr_auto] items-center gap-4">
                                <div class="flex flex-wrap items-center gap-2 min-w-0">
                                    ...
                                    <span class="flex items-center gap-1.5 rounded-lg border border-accent/50 bg-accent/10 px-3 py-1 text-xs font-bold text-accent">{{ $seat }}</span>
                                    @endforeach
                                </div>
                                <form method="POST" action="{{ route('cart.item.edit-begin') }}" class="flex-shrink-0">
                                    @csrf
                                    <input type="hidden" name="type" value="{{ $item['type'] }}">
                                    <input type="hidden" name="reference_id" value="{{ $item['reference_id'] }}">
                                    ...
                                    <input type="hidden" name="movie_url" value="{{ $item['url'] }}">
                                    <button type="submit" class="text-sm text-placeholder underline hover:text-accent">Change seats</button>
                                </form>
                            </div>

                            <div class="flex items-center justify-between pt-1 border-t border-border">
                                <span class="text-xs text-placeholder">{{ $item['quantity'] }} tickets × {{ number_format($item['price'], 2) }}€</span>
                                <span class="text-sm font-bold text-accent">{{ number_format($item['subtotal'], 2) }}€</span>
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            <section>
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <span class="rounded-full bg-button border border-border px-2.5 py-0.5 text-xs text-placeholder">{{ count($souvenirs) }} items</span>
                </div>
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex gap-4 rounded-2xl border border-border bg-dark p-4 shadow-[0_14px_36px_rgba(0,0,0,.35)] tablet:p-5">
                        ...
                        <div class="flex flex-1 flex-col gap-3 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <a href="{{ $item['url'] }}" class="block line-clamp-2 font-semibold hover:text-accent transition">{{ $item['name'] }}</a>
                            ...
                            <div class="flex items-center justify-between mt-auto">
                                <form method="POST" action="{{ route('cart.item.quantity') }}">
                                    @csrf
                                    <input type="hidden" name="type" value="{{ $item['type'] }}">
                                    <input type="hidden" name="reference_id" value="{{ $item['reference_id'] }}">
                                    <input type="hidden" name="options" value="{{ json_encode($item['options'] ?? []) }}">
                                    <div class="inline-flex h-9 items-center overflow-hidden rounded-xl border border-border bg-bg">
                                        <button type="submit" name="quantity" value="{{ $item['quantity'] - 1 }}"
                                            ...
                                            class="h-full px-3 text-sm font-semibold text-placeholder transition hover:text-accent disabled:opacity-50 disabled:cursor-not-allowed">−</button>
                                        <span class="h-full w-10 flex items-center justify-center text-sm font-bold">{{ $item['quantity'] }}</span>
                                        <button type="submit" name="quantity" value="{{ $item['quantity'] + 1 }}"
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-2 text-sm">
                        ...
                        <div class="flex justify-between gap-2">
                            <span class="truncate min-w-0 text-placeholder">{{ $item['name'] }} × {{ $item['quantity'] }}</span>
